      ...
      <div class="form-box">
        ...
        <div class="input-wrap">
          ...
          <input
            ...
            maxlength="12"
          />
          <span v-if="isUsernameError" class="error-text">用户名应为6-12位字母/数字</span>
        </div>
        ...
        <div class="input-wrap">
          ...
          />
          <span v-if="isPasswordError" class="error-text">密码应为6-12位字母/数字</span>
        </div>
        ...
        <div class="links">
          ...
          <router-link to="/phone-login">手机号登录</router-link>
        </div>
      </div>
    </div>
  </div>
  ...
  try {
    ...
    if (res.code === 1) {
      ...
      try {
        ...
        setTimeout(() => {
          // role=0为管理员，跳转到管理页面；role=1为用户，跳转到用户页面
          if (role === 0) {
        ...
        showTip('解析token失败', 'error')
      }
    } else {
      showTip(res.msg, 'error')
    }
  } catch (error) {
    showTip('网络异常，请重试', 'error')
  }
}
</script>
